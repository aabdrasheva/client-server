...
class Client extends JFrame implements ActionListener, MouseListener {
    JPanel panel;
    JLabel title, subT, error, servFiles;
    Font font, labelfont;
    JTextField txt;
    JButton down;
    String dirName;
    ...
    public Client(String dir, String host, int port) {
            ...
            try {
                ...
                while (complete) {
                    try {
                        ...
                        System.out.println("ВОТ ТУТ: " + exc.getMessage());
                        error.setText("ВОТ ТУТ: " + exc.getMessage());
                        panel.revalidate();
                    }
                }
                dataOut.close();
                ...
                System.out.println("Exception BUTTON: " + exc.getMessage());
                error.setText("Exception BUTTON:" + exc.getMessage());
                panel.revalidate();
            }
        });
        ...
        try {
            ...
            //String dirName = (String) oin.readObject();
    ...
    public static void main(String[] args) {
        if (args.length >= 3) {
            ...
            Client tcp = new Client(args[0], args[1], 3333);
            tcp.setSize(1000, 900);
            tcp.setVisible(true);
        } else if (args.length == 1) {
            Client tcp = new Client(args[0], "localhost", 3333);
            tcp.setSize(1000, 900);
            tcp.setVisible(true);
        } else {
